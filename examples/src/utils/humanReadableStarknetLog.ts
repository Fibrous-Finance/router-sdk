...
export function humanReadableStarknetSwapCallDataLog(
    ...
    const amountIn = uint256.uint256ToBN({
        low: callData.calldata[2],
        high: callData.calldata[3],
    });
    const minAmountOut = uint256.uint256ToBN({
        low: callData.calldata[4],
        high: callData.calldata[5],
    });
    const destination = callData.calldata[6];

    console.log(
        `  Amount In:     ${formatAmount(amountIn.toString(), Number(inputToken.decimals))} ${inputToken.symbol}`,
    );
    console.log(
        `  Min Amount Out: ${formatAmount(minAmountOut.toString(), Number(outputToken.decimals))} ${outputToken.symbol}`,
    );
