...
export async function monitorTransaction(tx: TransactionResponse): Promise<TransactionResult> {
    try {
      const receipt = await tx.wait();
      const explorerUrl = EXPLORER_MAP.find(chain => chain.chain_name == "hyperevm")?.explorer_url;
